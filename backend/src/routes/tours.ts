...
router.get('/', async (req: Request, res: Response, _next: NextFunction) => {
  try {
    ...
    console.warn('[API] tours findMany failed, serving LocalStore:', (error as Error).message);
  }
...
router.get('/:slug', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    console.warn('[API] tours findUnique failed, checking LocalStore:', (dbErr as Error).message);
  }
...
router.post('/', async (req: Request, res: Response) => {
  ...
  if (payload.precio_adulto !== undefined) payload.precio_adulto = Number(payload.precio_adulto) || 0;
  if (payload.precio_adulto_usd !== undefined) payload.precio_adulto_usd = Number(payload.precio_adulto_usd) || 0;
  if (payload.duracion_dias !== undefined) payload.duracion_dias = Number(payload.duracion_dias) || 1;
  if (payload.cupos_disponibles !== undefined) payload.cupos_disponibles = Number(payload.cupos_disponibles) || 10;

  let dbCreated: any = null;
  try {
    dbCreated = await prisma.tour.create({ data: payload });
  } catch (error) {
    console.warn('[API] prisma.tour.create failed, saving in LocalStore:', (error as Error).message);
  }
...
router.put('/:id', async (req: Request, res: Response) => {
  ...
  if (!isNaN(id)) {
    try {
      ...
      console.warn('[API] prisma.tour.update failed, updating in LocalStore:', (error as Error).message);
    }
  }
...
router.delete('/:id', async (req: Request, res: Response) => {
  ...
  if (!isNaN(id)) {
    try {
      ...
      console.warn('[API] prisma.tour.delete failed:', (error as Error).message);
    }
  }
