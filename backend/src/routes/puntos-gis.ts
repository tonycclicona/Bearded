...
router.get('/', async (req: Request, res: Response, _next: NextFunction) => {
  try {
    ...
    console.warn('[API] puntos-gis findMany failed, serving LocalStore:', (error as Error).message);
  }
...
router.get('/:slug', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    console.warn('[API] puntos-gis findUnique failed, checking LocalStore:', (dbErr as Error).message);
  }
...
router.post('/', async (req: Request, res: Response) => {
  ...
  if (payload.latitud !== undefined) payload.latitud = Number(payload.latitud) || 0;
  if (payload.longitud !== undefined) payload.longitud = Number(payload.longitud) || 0;
  if (payload.altitudMsnm !== undefined) payload.altitudMsnm = Number(payload.altitudMsnm) || 2800;
  if (payload.imageUrl && !payload.fotoUrl) payload.fotoUrl = payload.imageUrl;
  if (payload.fotoUrl && !payload.imageUrl) payload.imageUrl = payload.fotoUrl;

  let dbCreated: any = null;
  try {
    dbCreated = await prisma.puntoGIS.create({ data: payload });
  } catch (error) {
    console.warn('[API] prisma.puntoGIS.create failed, saving in LocalStore:', (error as Error).message);
  }
...
router.put('/:id', async (req: Request, res: Response) => {
  ...
  if (!isNaN(numId)) {
    try {
      ...
      console.warn('[API] prisma.puntoGIS.update failed, updating in LocalStore:', (error as Error).message);
    }
  }
...
router.delete('/:id', async (req: Request, res: Response) => {
  ...
  if (!isNaN(numId)) {
    try {
      ...
      console.warn('[API] prisma.puntoGIS.delete failed:', (error as Error).message);
    }
  }
