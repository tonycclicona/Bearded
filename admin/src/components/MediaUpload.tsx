...
import { UploadCloud, X, Image as ImageIcon, Music, Loader2, Link as LinkIcon, Plus } from 'lucide-react';
import { uploadApi, resolveMediaUrl } from '@/lib/api';

interface MediaUploadProps {
  label: string;
  mode?: 'image' | 'gallery' | 'audio';
  value: any;
...
export default function MediaUpload({
  ...
  const acceptTypes = isAudio ? 'audio/*' : 'image/*';
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="text-[11px] text-[#10352b] hover:underline flex items-center gap-1 font-medium"
        >
        ...
        <div className="pt-1">
          {isGallery ? (
            ...
            <input
              ...
              placeholder={isAudio ? 'https://.../canto.mp3' : 'https://.../foto.webp'}
              ...
              {value.map((url: string, index: number) => (
                <div key={index} className="relative group rounded-lg overflow-hidden border border-gray-200 aspect-video bg-gray-100">
                  ...
                  <button
                    ...
                    className="absolute top-1 right-1 bg-red-600/90 text-white rounded-full p-1 opacity-80 hover:opacity-100 transition-opacity shadow"
            ...
            <div className="relative group rounded-xl overflow-hidden border border-gray-200 bg-gray-50">
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                <button
                  type="button"
                  onClick={() => inputRef.current?.click()}
                  className="bg-white/90 hover:bg-white text-gray-900 text-xs font-semibold px-3 py-1.5 rounded-lg shadow transition-all"
                >
                  Cambiar foto
                </button>
                <button
